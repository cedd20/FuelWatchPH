...
const TYPE_CONFIG = {
  ...
  system: {
    icon: Info,
    iconBg: "from-gray-500/10 to-neutral-500/10",
              ...
              return (
                ...
                >
                  <div className="relative z-10 flex items-start gap-5 lg:gap-7">
                    <div
                      className={`w-12 h-12 lg:w-16 lg:h-16 bg-gradient-to-br ${TYPE_CONFIG[notification.type]?.iconBg || TYPE_CONFIG.system.iconBg} rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 transition-transform`}
                    >
                      {(() => {
                        const Icon = TYPE_CONFIG[notification.type]?.icon || TYPE_CONFIG.system.icon;
                        return <Icon className={`w-6 h-6 lg:w-8 lg:h-8 ${TYPE_CONFIG[notification.type]?.iconColor || TYPE_CONFIG.system.iconColor}`} strokeWidth={2.5} />;
                      })()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <h3 className={`font-bold text-base lg:text-xl tracking-tight transition-colors ${
                          notification.is_read ? "text-foreground" : "text-emerald-600 dark:text-emerald-400"
                        }`}>
                          {notification.title}
                        </h3>
                        {!notification.is_read && (
