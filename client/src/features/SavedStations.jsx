          ...
          <div className="max-w-2xl mx-auto relative z-10">
            <div className="flex items-center gap-6 mb-10">
              ...
              <div>
                ...
                <p className="mt-1 text-xs font-bold uppercase tracking-widest text-muted-foreground">Your favorites, always nearby</p>
              </div>
            </div>

            {/* Search */}
            <div className="relative">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" strokeWidth={2.5} />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search your favorites..."
                className="app-panel w-full rounded-[2rem] border border-emerald-500/10 py-5 pl-14 pr-6 text-sm font-bold placeholder:text-muted-foreground focus:border-emerald-500/30 focus:outline-none transition-all shadow-2xl"
              />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto px-6 -mt-12 space-y-4">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="app-panel h-28 animate-pulse rounded-[2rem] border border-emerald-500/5" />
            ))
